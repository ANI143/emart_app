import 'package:country_code_picker/country_code_picker.dart';
import 'package:emart_project/widget_common/textfield_with_contryflag/input_text_field.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

class NumberWithContrycode extends StatefulWidget {
  TextEditingController phoneController;
  NumberWithContrycode({required this.phoneController, super.key});

  @override
  State<NumberWithContrycode> createState() => _NumberWithContrycodeState();
}

class _NumberWithContrycodeState extends State<NumberWithContrycode> {
  final focusNode = FocusNode();
  // TextEditingController phoneController = TextEditingController();
  var data;
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(right: 10, left: 10),
      child: NumberWidget(
        focusNode: focusNode,
        inputFormatters: <TextInputFormatter>[
          FilteringTextInputFormatter.deny(RegExp("[^a-zA-Z0-9]"))
        ],
        hintText: 'Mobilenumber',
        hintStyle: const TextStyle(
          color: Colors.black,
          fontSize: 16,
          fontWeight: FontWeight.w400,
        ),
        prefixIconConstraints: BoxConstraints.loose(const Size(125, 50)),
        prefixIcon: Row(
          children: [
            CountryCodePicker(
              onChanged: (element) {
                element.toLongString();
                data = element.toLongString();
                print("data is that $data");
              },
              initialSelection: 'India',
              showCountryOnly: true,
              showFlagDialog: true,
              // showOnlyCountryWhenClosed: true,
              favorite: const ['+19', 'India'],
            ),
            const VerticalDivider(
              width: 2,
              color: Colors.black,
            )
          ],
        ),
        controller: widget.phoneController,
        keyboardType: TextInputType.number,
        maxLength: 10,
        validator: (value) {
          return validateMobile(
            value,
            mobileNumberLength: 10,
          );
        },
      ),
    );
  }

  static String? validateMobile(String? value, {int? mobileNumberLength}) {
    if (value == '' || value?.isEmpty == true) {
      return "Please Enter a Phone Number";
      //   // } else if (!RegExp(r'^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$').hasMatch(value)){
      //   //   return "Please Enter a Valid Phone Number";
      // }
      // if (mobileNumberLength != null) {
    } else if (value!.length < (mobileNumberLength ?? 9)) {
      return "Invalid Phone Number";
    } else if (value.contains(RegExp(r'[;*+_,#/()]'))) {
      return "Invalid Phone Number";
    }
    // }

    return null;
  }
}
